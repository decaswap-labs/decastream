...
/**
 * @title UpdatePancakeSwapFetcher
 * @notice Update the PancakeSwapFetcher contract safely with thorough dependency validation
 * @dev This script updates only the PancakeSwapFetcher while maintaining version history
 */
contract UpdatePancakeSwapFetcher is Script {
    ...
    function setUp() public {
        ...
        currentFetcher = vm.envAddress("PANCAKESWAP_FETCHER_ADDRESS");
    }
    
    function run() public {
        vm.startBroadcast();
        
        console.log("Starting PancakeSwapFetcher Update Process");
        ...
        // Step 3: Deploy new PancakeSwapFetcher
        console.log("Step 3: Deploying new PancakeSwapFetcher...");
        _deployNewFetcher();
        console.log("");
    ...
    function _validatePreUpdate() internal view {
        ...
        require(Address.isContract(currentFetcher), "Current PancakeSwapFetcher not found");
        require(Address.isContract(address(factory)), "CREATE2 Factory not found");
        
        // Verify current fetcher functionality
        PancakeSwapFetcher fetcher = PancakeSwapFetcher(currentFetcher);
        require(fetcher.router() == PANCAKESWAP_ROUTER, "Current fetcher router mismatch");
    ...
    function _backupCurrentState() internal view {
        ...
        console.log("  Current PancakeSwapFetcher:", currentFetcher);
        console.log("  Registry:", registry);
    ...
    function _deployNewFetcher() internal {
        ...
        bytes32 fetcherSalt = keccak256(abi.encodePacked(baseSalt, "PancakeSwapFetcher"));
        
        // Deploy new PancakeSwapFetcher
        newFetcher = factory.deployWithName(
            0, // No ETH sent
            fetcherSalt,
            type(PancakeSwapFetcher).creationCode,
            abi.encode(PANCAKESWAP_ROUTER),
            "PancakeSwapFetcher"
        );
        
        console.log("New PancakeSwapFetcher deployed at:", newFetcher);
    }
    
    function _verifyNewFetcher() internal view {
        console.log("Verifying new PancakeSwapFetcher:");
        
        PancakeSwapFetcher newFetcherContract = PancakeSwapFetcher(newFetcher);
        
        // Verify constructor parameters
        require(newFetcherContract.router() == PANCAKESWAP_ROUTER, "New fetcher router mismatch");
    ...
    function _validatePostUpdate() internal view {
        ...
        console.log("  Router address correct");
    ...
    function _updateVersionHistory() internal {
        ...
        string memory filename = string(abi.encodePacked("versions/update-pancakeswapfetcher-", timestamp, ".json"));
    ...
    function _createNewVersionData(string memory currentHistory) internal view returns (string memory) {
        ...
        versionData = string(abi.encodePacked(versionData, "  \"updated_contract\": \"PancakeSwapFetcher\",\n"));
        ...
        // Add PancakeSwapFetcher with updated history
        versionData = string(abi.encodePacked(versionData, "    \"PancakeSwapFetcher\": {\n"));
    ...
    function _displayUpdateSummary() internal view {
        console.log("PANCAKESWAPFETCHER UPDATE COMPLETE!");
        console.log("====================================");
        console.log("Network:", block.chainid);
        console.log("");
        console.log("Updated Contracts:");
        console.log("  PancakeSwapFetcher:", newFetcher);
        ...
        console.log("  PANCAKESWAP_FETCHER_ADDRESS=", newFetcher);
    }
}
